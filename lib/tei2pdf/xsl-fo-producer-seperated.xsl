...
<!-- Spezialfall: Bild in Tabelle -->
  ...
  <xsl:variable name="header_and_footer">
    <fo:static-content flow-name="before-even">
      ...
      <fo:block><xsl:call-template name="page_header"/>Book of Abstracts zur Konferenz Digital Humanities im deutschsprachigen Raum 2018</fo:block>
    </fo:static-content>
    <fo:static-content flow-name="after-even">
      <xsl:call-template name="header_and_footer-even"/>
      <fo:block><xsl:call-template name="page_footer"/>
        <!--
        <fo:page-number/>
        -->
        </fo:block>
